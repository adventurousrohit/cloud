...
const PriceCardWithTabs = ({ title, subtitle, linuxPricingData, windowsPricingData }) => {
  const { currency, convert } = useContext(CurrencyContext);
  const [selectedTab, setSelectedTab] = useState("India"); // Default region is "India"
  const [currentData, setCurrentData] = useState(linuxPricingData); // Default is Linux Pricing Data
  const [selectedPriceTab, setSelectedPriceTab] = useState("3 years");

  // Filter the pricing data based on the selected region
  const filteredData = currentData?.plan?.filter((plan) => plan.region === selectedTab);
              ...
              <div key={index} className="col-xl-3 col-md-6">
                <div className="price-card-item-one position-relative overflow-hidden bg-white px-7 py-9 border-end h-100">
                  <div className="discount-badge gradient-bg">
                    <p className="text-white fw-bold mb-0">{getDiscountText(selectedPriceTab)} Sale</p>
                  </div>
